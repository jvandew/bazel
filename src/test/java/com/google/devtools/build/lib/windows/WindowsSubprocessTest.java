...
public class WindowsSubprocessTest {
  ...
  @Test
  public void testEnvironmentSystemRootAndSystemDrive() throws Exception {
    assertThat(Strings.nullToEmpty(System.getenv("TZ"))).isNotEmpty();
    SubprocessBuilder subprocessBuilder = new SubprocessBuilder(WindowsSubprocessFactory.INSTANCE);
    subprocessBuilder.setWorkingDirectory(new File("."));
    subprocessBuilder.setArgv(
        ImmutableList.of(mockBinary, "-jar", mockSubprocess, "O$SYSTEMROOT", "O$SYSTEMDRIVE"));
    subprocessBuilder.setEnv(
        ImmutableMap.of("SYSTEMROOT", "systemroot", "SYSTEMDRIVE", "systemdrive"));
    process = subprocessBuilder.start();
    process.waitFor();
    assertThat(process.exitValue()).isEqualTo(0);

    byte[] buf = process.getInputStream().readAllBytes();
    assertThat(new String(buf, UTF_8).trim()).isEqualTo("systemrootsystemdrive");
  }
